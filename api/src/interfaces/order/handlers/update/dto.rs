use chrono::{DateTime, Utc};
use serde::{Deserialize, Serialize};
use uuid::Uuid;
use crate::domain::order::{Order, OrderItem};

#[derive(Deserialize)]
pub struct OrderItemUpdateRequestBody {
    pub product_id: Uuid,
    pub quantity:   u32,
}

#[derive(Deserialize)]
pub struct OrderUpdateRequestBody {
    pub customer_phone: Option<String>,
    #[serde(default)]
    pub customer_email: Option<Option<String>>,
    #[serde(default)]
    pub customer_name:  Option<Option<String>>,
    pub status:         Option<String>,
    pub items:          Option<Vec<OrderItemUpdateRequestBody>>,
}

#[derive(Serialize)]
pub struct OrderItemUpdateResponse {
    id:                  Uuid,
    price_cents_at_time: u32,
    product_id:          Uuid,
    product_name:        String,
    quantity:            u32,
    sale_mode:           String,
}

impl From<OrderItem> for OrderItemUpdateResponse {
    fn from(i: OrderItem) -> Self {
        Self {
            id:                  i.get_id().value(),
            price_cents_at_time: i.get_price_cents_at_time().value(),
            product_id:          i.get_product_id().value(),
            product_name:        i.get_product_name().to_string(),
            quantity:            i.get_quantity().value(),
            sale_mode:           i.get_sale_mode().as_str().to_string(),
        }
    }
}

#[derive(Serialize)]
pub struct OrderUpdateResponse {
    pub created_at:        DateTime<Utc>,
    pub customer_email:    Option<String>,
    pub customer_name:     Option<String>,
    pub customer_phone:    String,
    pub id:                Uuid,
    pub items:             Vec<OrderItemUpdateResponse>,
    pub status:            String,
    pub total_price_cents: u32,
}

impl From<(Order, Vec<OrderItem>)> for OrderUpdateResponse {
    fn from((o, items): (Order, Vec<OrderItem>)) -> Self {
        Self {
            created_at:        o.get_created_at(),
            customer_email:    o.get_customer_email().map(|s| s.to_string()),
            customer_name:     o.get_customer_name().map(|s| s.to_string()),
            customer_phone:    o.get_customer_phone().to_string(),
            id:                o.get_id().value(),
            items:             items.into_iter().map(OrderItemUpdateResponse::from).collect(),
            status:            o.get_status().as_str().to_string(),
            total_price_cents: o.get_total_price_cents().value(),
        }
    }
}
